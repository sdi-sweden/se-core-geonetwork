						...
						<xsl:choose>
							...
							<xsl:when test="string-length(.) = 7">
								<xsl:variable name="aux">
									<xsl:call-template name="dateWithLastDayOfMonth">
										<xsl:with-param name="given-date">
											<xsl:value-of select="." />-01</xsl:with-param>
									</xsl:call-template>
								</xsl:variable>
								<d>
									<xsl:value-of select="." />-<xsl:value-of select="substring($aux, 9, 2)" />
								</d>
							</xsl:when>

							<xsl:when test="string-length(.) = 4">
								<d>
									<xsl:value-of select="." />-12-31</d>
							</xsl:when>
						</xsl:choose>
